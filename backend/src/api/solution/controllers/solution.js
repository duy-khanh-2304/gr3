...
module.exports = createCoreController('api::solution.solution', ({strapi}) => ({
  async findOne(ctx){
    ...
    const entity = await strapi.service('api::solution.solution').find(query) 
    strapi.service('api::solution.solution').update(entity.results[0].id, {
      data: {
        seen_time_array: [
  ...
  async postComment(ctx){
    ...
    await strapi.service('api::solution.solution').update(entity.results[0].id, {
      data: {
        comment: [
